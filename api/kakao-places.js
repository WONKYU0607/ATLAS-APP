...
export default async function handler(req, res) {
  ...
  const { lat, lng, category, radius = '3000', size = '15', page = '1' } = req.query
  ...
  try {
    ...
    const url = `https://dapi.kakao.com/v2/local/search/category.json?category_group_code=${category}&x=${lng}&y=${lat}&radius=${radius}&size=${size}&page=${page}&sort=accuracy`
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: 'Kakao API 요청 실패', detail: errorText })
    }
