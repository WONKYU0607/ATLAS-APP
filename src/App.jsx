...
function App() {
  ...
  return (
    <div style={{width:'100vw',height:'100vh',overflow:'hidden',position:'relative',fontFamily:"'Pretendard','Inter',system-ui,sans-serif",background:'#000'}}>
                  ...
                  {(sidePanel === 'hotspots' ? hotspots : restaurants).map((place, idx) => (
                    <a key={idx}
                      ...
                      <div style={{display:'flex',gap:10,padding:10,alignItems:'center'}}>
                        {/* 번호 태그 */}
                        <div style={{padding:'3px 7px',background:'#0f172a',borderRadius:4,fontSize:10,fontWeight:800,color:'#f8fafc',flexShrink:0,minWidth:26,textAlign:'center',letterSpacing:'.5px',alignSelf:'flex-start',marginTop:2}}>
                        ...
                        {place.photos && place.photos.length > 0 ? (
                          ...
                          <div style={{width:62,height:62,borderRadius:7,background:'#f1f5f9',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#94a3b8',flexShrink:0,letterSpacing:'.3px'}}>
                        ...
                        <div style={{display:'flex',flexDirection:'column',gap:4,flexShrink:0}}>
                          <button onClick={e=>{e.preventDefault();e.stopPropagation();addToCourse({source:sidePanel==='hotspots'?'hotspot':'restaurant',name:place.name,displayName:place.name,cityName:selectedCity?._koName||selectedCity?.name,cityDisplayName:getCityName(selectedCity?._koName||selectedCity?.name),rating:place.rating,place_id:place.place_id,vicinity:place.vicinity,lat:selectedCity?.lat,lng:selectedCity?.lng,emoji:sidePanel==='hotspots'?'📍':foodCategory==='cafe'?'☕':foodCategory==='bar'?'🍻':'🍽️',photo_ref:place.photos?.[0]?.photo_reference||null})}}
                            style={{background:isInCourse(place.name,sidePanel==='hotspots'?'hotspot':'restaurant')?'#0f172a':'#f8fafc',border:isInCourse(place.name,sidePanel==='hotspots'?'hotspot':'restaurant')?'1.5px solid #0f172a':'1.5px solid #e2e8f0',color:isInCourse(place.name,sidePanel==='hotspots'?'hotspot':'restaurant')?'white':'#cbd5e1',width:28,height:28,borderRadius:6,cursor:'pointer',fontSize:13,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',transition:'all .2s'}}
                          ...
                          <button onClick={e=>{e.preventDefault();e.stopPropagation();toggleFav({type:sidePanel==='hotspots'?'hotspot':'restaurant',name:place.name,place_id:place.place_id,rating:place.rating,user_ratings_total:place.user_ratings_total,vicinity:place.vicinity,cityDisplayName:getCityName(selectedCity?._koName||selectedCity?.name)})}}
                            style={{background:'#f8fafc',border:isFav(sidePanel==='hotspots'?'hotspot':'restaurant',place.name)?'1.5px solid #fbbf24':'1.5px solid #e2e8f0',color:isFav(sidePanel==='hotspots'?'hotspot':'restaurant',place.name)?'#f59e0b':'#cbd5e1',width:28,height:28,borderRadius:6,cursor:'pointer',fontSize:12,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',transition:'all .2s'}}
                            title={t("favToggle")}>♥</button>
                        </div>
                      </div>
                    </a>
                  ))}
            ...
            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:12}}>
              ...
              <div style={{display:'flex',gap:6,flexShrink:0}}>
                ...
                <button onClick={()=>toggleFav({type:'city',name:selectedCity?._koName||selectedCity?.name,_koName:selectedCity?._koName||selectedCity?.name,displayName:getCityName(selectedCity?._koName||selectedCity?.name),emoji:selectedCity?.emoji,color:selectedCity?.color,countryEn:selectedCity?.countryEn,countryName:countryKo,lat:selectedCity?.lat,lng:selectedCity?.lng})}
                  style={{background:isFav('city',selectedCity?._koName||selectedCity?.name)?'rgba(251,191,36,.15)':'#f1f5f9',border:isFav('city',selectedCity?._koName||selectedCity?.name)?'1.5px solid #fbbf24':'1.5px solid #e2e8f0',color:isFav('city',selectedCity?._koName||selectedCity?.name)?'#f59e0b':'#94a3b8',width:34,height:34,borderRadius:9,cursor:'pointer',fontSize:14,display:'flex',alignItems:'center',justifyContent:'center',transition:'all .2s'}}
                  title={t("favToggle")}>♥</button>
                          ...
                          {cityData.spots.map((spot,i)=>(
                            <div key={i} className="card"
                              ...
                              <div style={{height: selectedSpot?.name===spot.name ? 200 : 142,overflow:'hidden',position:'relative',transition:'height .3s'}}>
                                ...
                                {/* 번호 태그 */}
                                <div style={{position:'absolute',top:8,left:8,padding:'3px 7px',background:'rgba(0,0,0,.75)',backdropFilter:'blur(4px)',borderRadius:4,fontSize:10,fontWeight:800,color:'#fff',letterSpacing:'.5px',zIndex:2}}>
                                ...
                                <button onClick={e=>{e.stopPropagation();toggleFav({type:'spot',name:spot.name,cityName:selectedCity?._koName||selectedCity?.name,cityDisplayName:getCityName(selectedCity?._koName||selectedCity?.name),wikiTitle:spot.wikiTitle,spotType:spot.type,rating:spot.rating})}}
                                  style={{position:'absolute',top:8,right:8,width:30,height:30,borderRadius:7,background:isFav('spot',spot.name)?'rgba(251,191,36,.9)':'rgba(0,0,0,.45)',border:'none',color:isFav('spot',spot.name)?'white':'rgba(255,255,255,.6)',fontSize:13,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(4px)',transition:'all .2s',zIndex:2}}
                                  title={t("favToggle")}>♥</button>
      ...
      {/* ── 코스 플래너 패널 (Dark Minimal) ── */}
      {showCoursePlanner && courseDays.length > 0 && (
        <div style={{position:'absolute',top:72,left:0,bottom:0,width:Math.min(500,typeof window!=='undefined'?window.innerWidth-30:480),zIndex:1100,background:'#0c0c0c',borderRight:'1px solid #1f1f1f',boxShadow:'20px 0 60px rgba(0,0,0,.4)',display:'flex',flexDirection:'column',animation:'coursePlannerIn .35s cubic-bezier(.16,1,.3,1)'}}>

          {/* 헤더 */}
          <div style={{padding:'20px 20px 0',flexShrink:0}}>
            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:18}}>
              <div>
                <div style={{fontSize:20,fontWeight:700,color:'#f5f5f5',letterSpacing:'-.5px'}}>{t('coursePlanner')}</div>
                <div style={{fontSize:11,color:'#444',marginTop:3,letterSpacing:'.1px'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:6}}>
                <button
                  ...
                  style={{padding:'5px 10px',borderRadius:6,border:'1px solid #2a2a2a',background:'none',fontSize:11,fontWeight:500,color:'#555',cursor:'pointer',letterSpacing:'.1px',transition:'all .15s'}}
                  onMouseEnter={e=>{e.currentTarget.style.color='#ef4444';e.currentTarget.style.borderColor='#3a1515'}}
                  onMouseLeave={e=>{e.currentTarget.style.color='#555';e.currentTarget.style.borderColor='#2a2a2a'}}
                >{t('courseDeleteAll')}</button>
                <button onClick={()=>setShowCoursePlanner(false)}
                  style={{width:30,height:30,borderRadius:6,border:'1px solid #2a2a2a',background:'none',color:'#555',cursor:'pointer',fontSize:13,display:'flex',alignItems:'center',justifyContent:'center',transition:'all .15s'}}
            ...
            {/* 날짜 */}
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,padding:'8px 12px',background:'#141414',borderRadius:8,border:'1px solid #1f1f1f'}}>
              <span style={{fontSize:11,color:'#444',fontWeight:500,flexShrink:0}}>{t('courseDeparture')}</span>
              <input type="date" value={courseTripStart} onChange={e=>saveTripStart(e.target.value)}
                style={{flex:1,fontSize:11,border:'none',background:'none',color:'#aaa',fontWeight:600,outline:'none',cursor:'pointer',colorScheme:'dark'}}/>
              {courseTripStart && <button onClick={()=>saveTripStart('')} style={{background:'none',border:'none',color:'#333',fontSize:11,cursor:'pointer',lineHeight:1}}>✕</button>}
            </div>

            {/* 이동수단 */}
            <div style={{display:'flex',gap:2,marginBottom:14,background:'#141414',borderRadius:8,padding:3,border:'1px solid #1f1f1f'}}>
              {[{key:'transit',label:t('courseTransit'),sym:'⇄'},{key:'walking',label:t('courseWalking'),sym:'↑'},{key:'driving',label:t('courseDriving'),sym:'▶'}].map(m=>(
                <button key={m.key} onClick={()=>setCourseTransport(m.key)} style={{
                  flex:1,padding:'6px 0',fontSize:11,fontWeight:600,
                  background:courseTransport===m.key?'#1f1f1f':'none',
                  color:courseTransport===m.key?'#f0f0f0':'#444',
                  border:'none',borderRadius:6,cursor:'pointer',
                  display:'flex',alignItems:'center',justifyContent:'center',gap:5,transition:'all .15s'
                }}><span style={{fontSize:12}}>{m.sym}</span>{m.label}</button>
              ))}
            </div>

            {/* Day 탭 */}
            <div style={{display:'flex',gap:4,overflowX:'auto',paddingBottom:16,borderBottom:'1px solid #1a1a1a'}}>
              {courseDays.map((_,i)=>(
                <button key={i} onClick={()=>setActiveDayTab(i)} style={{
                  padding:'5px 14px',fontSize:11,fontWeight:activeDayTab===i?700:400,
                  background:activeDayTab===i?'#f5f5f5':'none',
                  color:activeDayTab===i?'#0c0c0c':'#444',
                  border:activeDayTab===i?'none':'1px solid #222',
                  borderRadius:20,cursor:'pointer',whiteSpace:'nowrap',transition:'all .15s',flexShrink:0
                }}>
                  Day {i+1}
                  {courseTripStart && <span style={{fontSize:9,opacity:.6,marginLeft:4}}>{formatDate(getDayDate(i))}</span>}
                  <span style={{fontSize:9,opacity:.5,marginLeft:4}}>({courseDays[i].items.length})</span>
                </button>
              ))}
              <button onClick={addCourseDay}
                style={{padding:'5px 12px',fontSize:13,background:'none',color:'#333',border:'1px dashed #2a2a2a',borderRadius:20,cursor:'pointer',fontWeight:500,flexShrink:0,transition:'all .15s'}}
                onMouseEnter={e=>{e.currentTarget.style.color='#888';e.currentTarget.style.borderColor='#444'}}
                onMouseLeave={e=>{e.currentTarget.style.color='#333';e.currentTarget.style.borderColor='#2a2a2a'}}>＋</button>
            </div>
          </div>
          ...
          <div style={{flex:1,overflowY:'auto',padding:'16px 20px'}}>
            {(() => {
              ...
              if (items.length === 0) return (
                <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:200,color:'#333',gap:8}}>
                  <div style={{width:40,height:40,border:'1px dashed #2a2a2a',borderRadius:10,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,color:'#2a2a2a'}}>+</div>
                  <span style={{fontSize:12,color:'#3a3a3a'}}>{t('courseEmptyTitle')}</span>
                  <span style={{fontSize:11,color:'#2a2a2a'}}>{t('courseEmptyDesc')}</span>
                </div>
              )
              let totalSec = 0
              ...
              return (
                <>
                  ...
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      <span style={{fontSize:12,fontWeight:700,color:'#f5f5f5'}}>Day {activeDayTab+1}</span>
                      {courseTripStart && <span style={{fontSize:11,color:'#444'}}>{formatDate(getDayDate(activeDayTab))}</span>}
                      <span style={{fontSize:11,color:'#333'}}>{items.length}{t('coursePlace')}</span>
                    </div>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      {totalMin > 0 && <span style={{fontSize:11,color:'#555',fontWeight:500}}>{totalHr > 0 ? `${totalHr}${t('courseHour')} ${totalMinRem}${t('courseMin')}` : `${totalMin}${t('courseMin')}`}</span>}
                      {loadingRoutes && <div style={{width:12,height:12,borderRadius:'50%',border:'1.5px solid #2a2a2a',borderTopColor:'#555',animation:'spin .7s linear infinite'}}/>}
                      {courseDays.length > 1 && (
                        <button onClick={()=>removeCourseDay(activeDayTab)}
                          style={{fontSize:10,background:'none',border:'1px solid #2a2a2a',color:'#444',padding:'3px 8px',borderRadius:5,cursor:'pointer',transition:'all .15s'}}
                          onMouseEnter={e=>{e.currentTarget.style.color='#ef4444';e.currentTarget.style.borderColor='#3a1515'}}
                  ...
                  {items.map((item, idx) => {
                    ...
                    const srcColor = item.source==='spot'?'#2a3a2a':item.source==='hotspot'?'#2a2a1a':'#1a2a2a'
                    const srcText = item.source==='spot'?'#4a7a4a':item.source==='hotspot'?'#7a6a2a':'#2a6a6a'
                    return (
                      <div key={item.name+'-'+idx}>
                        <div
                          ...
                          onDragOver={e=>{e.preventDefault();e.currentTarget.style.background='#1a1a1a'}}
                          onDragLeave={e=>e.currentTarget.style.background='#141414'}
                          onDrop={e=>{e.preventDefault();e.currentTarget.style.background='#141414';try{const from=JSON.parse(e.dataTransfer.getData('text/plain'));if(from.dayIdx===activeDayTab)reorderInDay(activeDayTab,from.itemIdx,idx);else moveToDayFn(from.dayIdx,from.itemIdx,activeDayTab)}catch{};setDragItem(null)}}
                          onDragEnd={()=>{setDragItem(null)}}
                          style={{
                            display:'flex',alignItems:'center',gap:10,padding:'11px 12px',
                            background:'#141414',borderRadius:10,border:'1px solid #1f1f1f',
                            cursor:'grab',transition:'background .1s',
                            opacity:dragItem?.dayIdx===activeDayTab&&dragItem?.itemIdx===idx?0.3:1
                          }}
                        >
                          {/* 번호 태그 */}
                          <div style={{padding:'3px 7px',background:'#f5f5f5',borderRadius:4,fontSize:10,fontWeight:800,color:'#0c0c0c',flexShrink:0,minWidth:24,textAlign:'center',letterSpacing:'.5px'}}>
                            {String(idx+1).padStart(2,'0')}
                          </div>
                          {/* 드래그 핸들 */}
                          <span style={{fontSize:12,color:'#2a2a2a',flexShrink:0,cursor:'grab',userSelect:'none',letterSpacing:2}}>⠿</span>
                          {/* 정보 */}
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{fontSize:13,fontWeight:600,color:'#e5e5e5',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{getCourseItemName(item)}</div>
                            <div style={{display:'flex',alignItems:'center',gap:5,marginTop:3}}>
                              <span style={{fontSize:10,color:'#444'}}>{getCourseItemCity(item)}</span>
                              <span style={{fontSize:9,padding:'1px 5px',borderRadius:3,background:srcColor,color:srcText,fontWeight:600}}>
                                {item.source==='spot'?t('courseSpot'):item.source==='hotspot'?t('courseHotspot'):t('courseRestaurant')}
                              </span>
                              {item.rating && <span style={{fontSize:9,color:'#555'}}>★{item.rating}</span>}
                            </div>
                          </div>
                          {/* Day 이동 */}
                          {courseDays.length > 1 && (
                            <select value="" onChange={e=>{if(e.target.value!=='')moveToDayFn(activeDayTab,idx,parseInt(e.target.value));e.target.value=''}}
                              style={{width:56,fontSize:9,padding:'3px 2px',border:'1px solid #2a2a2a',borderRadius:5,color:'#555',background:'#0c0c0c',cursor:'pointer',flexShrink:0}}>
                          ...
                          <button onClick={()=>removeFromDay(activeDayTab,idx)}
                            style={{background:'none',border:'none',color:'#2a2a2a',width:26,height:26,borderRadius:5,cursor:'pointer',fontSize:12,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',transition:'all .15s'}}
                            onMouseEnter={e=>{e.currentTarget.style.color='#ef4444';e.currentTarget.style.background='#1a0a0a'}}
                            onMouseLeave={e=>{e.currentTarget.style.color='#2a2a2a';e.currentTarget.style.background='none'}}>✕</button>
                        </div>

                        {/* 경로 */}
                        {idx < items.length - 1 && (
                          <div style={{display:'flex',alignItems:'center',gap:6,padding:'5px 0 5px 42px'}}>
                            <div style={{width:1,height:16,background:'#1f1f1f',marginRight:2}}/>
                            {route ? (
                              <span style={{fontSize:10,color:'#3a3a3a',fontWeight:500}}>
                                {route.noRoute ? t('courseNoRoute') : `${route.duration} · ${route.distance}`}
                              </span>
                            ) : loadingRoutes ? (
                              <div style={{width:10,height:10,borderRadius:'50%',border:'1.5px solid #222',borderTopColor:'#444',animation:'spin .7s linear infinite'}}/>
                            ) : (
                              <span style={{fontSize:10,color:'#222'}}>{t('courseCalc')}</span>
                            )}
                          </div>
                        )}
                      </div>
          ...
          {/* 푸터 */}
          <div style={{padding:'14px 20px',borderTop:'1px solid #1a1a1a',flexShrink:0,display:'flex',gap:6}}>
            {courseSource === 'ai' ? (
              <div style={{flex:1,padding:'11px',background:'#141414',border:'1px solid #2a2a2a',borderRadius:8,fontSize:12,fontWeight:600,color:'#555',display:'flex',alignItems:'center',justifyContent:'center',gap:6,letterSpacing:'.1px'}}>
              ...
              <button onClick={()=>{const s=saveCourseToList('manual');if(s)alert(t('courseSaved'));setShowCoursePlanner(false)}}
                style={{flex:1,padding:'11px',background:'#f5f5f5',border:'none',borderRadius:8,fontSize:12,fontWeight:700,color:'#0c0c0c',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6,letterSpacing:'.1px',transition:'all .15s'}}
            ...
            <button onClick={()=>setShowCoursePlanner(false)}
              style={{padding:'11px 18px',background:'none',border:'1px solid #1f1f1f',borderRadius:8,fontSize:12,fontWeight:500,color:'#444',cursor:'pointer',transition:'all .15s'}}
              onMouseEnter={e=>{e.currentTarget.style.background='#141414';e.currentTarget.style.color='#888'}}
              onMouseLeave={e=>{e.currentTarget.style.background='none';e.currentTarget.style.color='#444'}}>✕</button>
          </div>
        </div>
      )}
