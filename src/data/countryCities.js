...
export const COUNTRY_CITIES = {
  ...
  // 해외 영토·속령·자치 지역 (geojson에 별도 feature로 존재)
  "Greenland": [{ name:"누크", lat:64.18, lng:-51.74, emoji:"🏔️", color:"#3498db" }],
  "Puerto Rico": [{ name:"산후안", lat:18.47, lng:-66.12, emoji:"🏖️", color:"#e74c3c" }],
  "New Caledonia": [{ name:"누메아", lat:-22.28, lng:166.46, emoji:"🏝️", color:"#2ecc71" }],
  "Fr. Polynesia": [{ name:"파페에테", lat:-17.54, lng:-149.57, emoji:"🏝️", color:"#e91e63" }],
  "Cook Is.": [{ name:"아바루아", lat:-21.21, lng:-159.78, emoji:"🏝️", color:"#f39c12" }],
  "Aruba": [{ name:"오라녜스타트", lat:12.52, lng:-70.04, emoji:"🏝️", color:"#f39c12" }],
  "Curaçao": [{ name:"빌렘스타트", lat:12.11, lng:-68.93, emoji:"🏝️", color:"#3498db" }],
  "Cayman Is.": [{ name:"조지타운", lat:19.30, lng:-81.39, emoji:"🏝️", color:"#16a085" }],
  "Bermuda": [{ name:"해밀턴", lat:32.29, lng:-64.78, emoji:"🏝️", color:"#2980b9" }],
  "Faeroe Is.": [{ name:"토르스하운", lat:62.01, lng:-6.78, emoji:"🏔️", color:"#2c3e50" }],
  "Somaliland": [{ name:"하르게이사", lat:9.56, lng:44.07, emoji:"🏜️", color:"#e67e22" }],
  "W. Sahara": [{ name:"엘아이운", lat:27.15, lng:-13.20, emoji:"🏜️", color:"#d35400" }],
  "Niue": [{ name:"알로피", lat:-19.05, lng:-169.92, emoji:"🏝️", color:"#27ae60" }],
  "Hong Kong": [{ name:"홍콩", lat:22.35, lng:114.15, emoji:"🌃", color:"#2980b9" }],
  "Macao": [{ name:"마카오", lat:22.20, lng:113.56, emoji:"🎲", color:"#f39c12" }],
  "Anguilla": [{ name:"더밸리", lat:18.22, lng:-63.06, emoji:"🏝️", color:"#3498db" }],
  "Falkland Is.": [{ name:"스탠리", lat:-51.69, lng:-57.85, emoji:"🏔️", color:"#2980b9" }],
  "Turks and Caicos Is.": [{ name:"콕번타운", lat:21.46, lng:-71.14, emoji:"🏝️", color:"#1abc9c" }],
  "British Virgin Is.": [{ name:"로드타운", lat:18.42, lng:-64.62, emoji:"🏝️", color:"#2ecc71" }],
  "U.S. Virgin Is.": [{ name:"샬럿아말리에", lat:18.34, lng:-64.93, emoji:"🏝️", color:"#e74c3c" }],
  "Guam": [{ name:"하갓냐", lat:13.48, lng:144.79, emoji:"🏝️", color:"#8e44ad" }],
  "N. Mariana Is.": [{ name:"사이판", lat:15.19, lng:145.75, emoji:"🏝️", color:"#16a085" }],
  "American Samoa": [{ name:"파고파고", lat:-14.28, lng:-170.70, emoji:"🏝️", color:"#c0392b" }],
  "Jersey": [{ name:"세인트헬리어", lat:49.19, lng:-2.11, emoji:"🏝️", color:"#e67e22" }],
  "Guernsey": [{ name:"세인트피터포트", lat:49.46, lng:-2.54, emoji:"🏝️", color:"#27ae60" }],
  "Isle of Man": [{ name:"더글러스", lat:54.15, lng:-4.48, emoji:"🏝️", color:"#8e44ad" }],
  "Montserrat": [{ name:"플리머스", lat:16.71, lng:-62.22, emoji:"🏝️", color:"#d35400" }],
  "N. Cyprus": [{ name:"북니코시아", lat:35.20, lng:33.34, emoji:"🏛️", color:"#e74c3c" }],
  "Sint Maarten": [{ name:"필립스뷔르흐", lat:18.03, lng:-63.05, emoji:"🏝️", color:"#3498db" }],
  "St-Martin": [{ name:"마리고", lat:18.07, lng:-63.08, emoji:"🏝️", color:"#e91e63" }],
  "St-Barthélemy": [{ name:"귀스타비아", lat:17.90, lng:-62.83, emoji:"🏝️", color:"#f39c12" }],
  "Saint Helena": [{ name:"제임스타운", lat:-15.94, lng:-5.72, emoji:"🏝️", color:"#2c3e50" }],
}

// Natural Earth geojson의 축약형 국가명 → 정식 키로 연결하는 별칭
// (폴리곤 feat.properties.NAME과 COUNTRY_CITIES 키가 달라 매칭 실패하던 문제 해결)
const NAME_ALIASES = {
